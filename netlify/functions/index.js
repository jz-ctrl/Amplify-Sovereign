...
exports.handler = async (event, context) => {
    // 1. IDENTITY & AUTHORITY (NETLIFY ENV ONLY)
    ...
    // 2. SOVEREIGN TERMINAL (ADA COMPLIANT)
...
<head>
    ...
    <style>
        ...
        button { background: #39FF14; color: #000; width: 100%; padding: 25px; font-size: 2rem; font-weight: bold; border: none; cursor: pointer; border-radius: 5px; }
...
<body>
    <div class="terminal">
        ...
        <button onclick="executeSync()">EXECUTE SOVEREIGN SYNC</button>
        
        <div class="log-area" id="log">
            [SYSTEM READY] Google 2.0 ID: \${G_2_0_ID}<br>
            [STATUS] Netlify Protocol Active. ADA Override 100%.<br>
            [H2O] Handshake ready for miraclespritz.net.
        </div>
    </div>
    <script>
        async function executeSync() {
            const log = document.getElementById('log');
            log.innerHTML += "<br>[" + new Date().toLocaleTimeString() + "] [ACTION] Contacting Shopify...";
            try {
                ...
                log.innerHTML += "<br>[ERROR] Handshake failure. Verify Netlify Env Keys.";
            }
        }
    </script>
</body>
    ...
    try {
        ...
        return { 
            statusCode: 500, 
            body: JSON.stringify({ error: "EXECUTION_FAILURE", detail: err.message }) 
        };
    }
};
